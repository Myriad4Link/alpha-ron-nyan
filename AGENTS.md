...
- `MatchEngine`: Stateless processor containing pure business logic (action validation, turn progression, available actions computation).
...
- `RoundWindRotationRule`: Specifies the rotation rule of round wind, including the range of each wind (e.g., East 1..3, South 1..3).
...
1. Implement the relevant `Rule` interface (`WallGenerationRule`, `PlayerWindRotationRule`, `RoundWindRotationRule`, `YakuRule`).
...
**Implementation note**: Match logic is split between `MatchState` (mutable state), `MatchEngine` (pure business logic), and `Match` (public facade with listeners).
