...
class BindingTest : FunSpec({
    ...
    test("bind should catch generic exceptions and wrap them in BindException") {
        ...
        result.shouldBeInstanceOf<Result.Failure<ConfigurationError.GenericConfigurationError.InvalidConfiguration>>()
        ((result as Result.Failure).error as ConfigurationError.GenericConfigurationError.InvalidConfiguration).message shouldBe "Wrapped: Original error"
    }

    test("bind should propagate success value") {
        val result = binding<Int, ArnyanError> {
            binding({ ConfigurationError.GenericConfigurationError.InvalidConfiguration("Should not happen") }) {
                42
            }
        }
