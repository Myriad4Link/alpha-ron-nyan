...
class StandardShuntsuStrategyTest : FunSpec({
    ...
    test("tryRemove should return non-null and decrement histogram for valid shuntsu") {
    ...
    test("tryRemove should return null when mask negative (not-continuous tile type)") {
        val histogram = histogramOf(
    ...
    test("tryRemove should return null when mask mismatch across indices") {
    ...
    test("tryRemove should return null when index out of bounds (index > histogram.size - 3)") {
    ...
    test("revert should restore histogram after successful tryRemove") {
        ...
        StandardShuntsuStrategy.revert(histogram, removed!!)
